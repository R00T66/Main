...
local Settings = {
    ...
    ["Information"] = {
        ["MODS"] = {
            ["UNDETECTED"] = {147536033, 113825488},
    ...
    if isfile(SaveFile) then
        ...
        if Contents ~= nil then
            if Contents["MainSettings"] ~= nil then
                ...
                StarterGui:SetCore(
                    "SendNotification",
                    {Title = "WELCOME", Text = "Welcome to META WARE V2!", Duration = 15}
                )
                return
            end
            ...
            if Contents["Misc"] == nil then
                ...
                StarterGui:SetCore(
                    "SendNotification",
                    {Title = "ERROR", Text = "INVALID SETTINGS! They have been reset.", Duration = 30}
                )
                return
            end
        end
...
local SMA = function(Player, Role)
    ...
    AlertSound.Volume = 1
    AlertSound.SoundId = "rbxassetid://9117113714"
    AlertSound.Parent = game:GetService("CoreGui")
    AlertSound:Play()
...
local MainService = function(...)
    ...
    if
        (Flying == true and Client.Character and Client.Character:FindFirstChild("Torso") and
            Client.Character:FindFirstChild("HumanoidRootPart"))
     then
        UpdateFlight()
    end

    if #TrinketFolder:GetChildren() > 400 and not Disabled then
        Disabled = true
    end

    for i, v in pairs(TrinketFolder:GetChildren()) do
        if (AutoPI or TriESP) and not (Disabled) then
            ...
            if PrimaryPart ~= nil then
                if PrimaryPart.Transparency == 0 then
                    if not v:FindFirstChild("EyeLol") then
                        ...
                        Signal =
                            PrimaryPart:GetPropertyChangedSignal("Transparency"):Connect(
                            function(...)
                                if PrimaryPart.Transparency ~= 0 then
                                    Box:Remove()
                                    Signal:Disconnect()
...
local MainHook = function(self, ...)
    ...
    if Method == "FireServer" then
        if (self.Parent.Name == "FallDamage" and Settings["Save"]["Misc"]["NF"]) then
            return
        end
    end
...
local Check = function(Player)
    ...
    if InGroup ~= false and InGroup ~= nil then
        ...
        if table.find(Settings["Information"]["MODS"]["ROLES"], Role:lower()) then
            return Role:upper()
        else
            return false
        end
...
ESP_ENABLE:addToggle(
    "Trinkets",
    ESP_ENABLE_TABLE["Enabled_Trinket"],
    function(Value)
...
ESP_SETTINGS:addColorPicker(
    "PLAYER COLOR",
    Color3.fromRGB(PLRColor[1], PLRColor[2], PLRColor[3]),
    function(Value)
        ...
        ESP.Color =
            Color3.fromRGB(
            ESP_COLOR_TABLE["Color_Player"][1],
            ESP_COLOR_TABLE["Color_Player"][2],
            ESP_COLOR_TABLE["Color_Player"][3]
        )
        Save_Settings()
    end
)
...
ESP_SETTINGS:addColorPicker(
    "TRINKET COLOR",
    Color3.fromRGB(TRNColor[1], TRNColor[2], TRNColor[3]),
    function(Value)
...
ESP_SETTINGS:addColorPicker(
    "EYES COLOR",
    Color3.fromRGB(EYEColor[1], EYEColor[2], EYEColor[3]),
    function(Value)
...
StarterGui:SetCore(
    ...
    {
        Title = "ALERT",
        Text = "TRINKET ESP HAS BEEN DISABLED, REASON: [REGION]",
        Duration = 30
    }
)
